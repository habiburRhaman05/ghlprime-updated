...
const dataDir = resolve(__dirname, '_batch2_data')
...
const { data, error } = await supabase
  ...
  .select('id,slug,category,published,published_at')
...
for (const row of data) {
  console.log(
    `- slug=${row.slug} | category=${row.category} | published=${row.published} | published_at=${row.published_at} | id=${row.id}`
  )
}
