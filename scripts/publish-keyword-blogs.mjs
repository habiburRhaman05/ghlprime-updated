// Publish the June 2026 keyword-targeted blog batch (16 posts) to Supabase.
//
// Usage (PowerShell):
//   $env:SEED_EMAIL='you@example.com'; $env:SEED_PASSWORD='secret'; node ./scripts/publish-keyword-blogs.mjs
// Usage (bash):
//   SEED_EMAIL='you@example.com' SEED_PASSWORD='secret' node ./scripts/publish-keyword-blogs.mjs
//
...
import { keywordBlogPosts } from '../src/data/keywordBlogPosts.js'
...
const rows = keywordBlogPosts.map((post) => {
  const row = {}
...
console.log(`Published ${data.length} keyword blog posts:`)
for (const r of data.sort((a, b) => new Date(b.published_at) - new Date(a.published_at))) {
