              ...
              {socialLinks.length ? (
                <div className="leader-social-links">
                  {socialLinks.map(({ key, label, svg }) => (
                    <a key={key} href={leader[key]} target="_blank" rel="noopener noreferrer" className="leader-social-link" aria-label={`${leader.name} on ${label}`} title={`${leader.name} on ${label}`}>
                      {svg}
                    </a>
                  ))}
                </div>
